// more-or-less /sys/src/cmd/acid/hobbitdas.c
...
oneparcel(Parcel p)
{
        ...
        if (opcode == 0x02) {
                ...
                dprint("%-10sR%d", stackop[opcode].mnemonic, offset);
        }
        else if (opcode == 0x0B) {
                offset = p & 0x3FF;
                if (offset >= sizeof(nilop)/sizeof(Opcode))
                        dprint("niladic %lx", offset);
                else
                        dprint("%s", nilop[offset].mnemonic);
        }
        else {
                o = &onepop[opcode];
                dprint("%-10s", o->mnemonic);
                if (o->f0)
                        (*o->f0)(p, src);
                if (o->f1) {
                        dprint(",");
...
flow32(Parcel p, long w)
{
        ...
        default:
                dprint("mode %lx, operand %lx", (p>>4) & 0x0F, w);
                break;
        }
}
